...
void generate_limits_array(long* arr, long l_max, long l_min)
{
	for (int i = 0; i < l_max - l_min; i++)
	{
		arr[i] = l_max - i;
	}
}
...
int main()
{
	...
	generate_limits_array(arr, l_max, l_min);
	//print_array(arr, l_max - l_min);
	...
	printf("Time taken insert: %f\n", (double)(end - start) / CLOCKS_PER_SEC);

	generate_limits_array(arr, l_max, l_min);
	start = clock();
	select_sort(arr, n);
	end = clock();
	printf("Time taken select: %f\n", (double)(end - start) / CLOCKS_PER_SEC);

	generate_limits_array(arr, l_max, l_min);
	start = clock();
	bubble_sort(arr, n);
	end = clock();
	printf("Time taken bubble: %f\n", (double)(end - start) / CLOCKS_PER_SEC);
